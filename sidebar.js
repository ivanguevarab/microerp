...
(function () {
            ...
            groups: [
                {
                    ...
                    items: [
                        ...
                        { label: 'Cuentas por Pagar', url: 'cuentas_por_pagar.html' }
                    ]
                },
                {
            ...
            groups: [
                {
                    ...
                    items: [
                        ...
                        { label: 'Importador Ventas Terceros', url: 'importador_ventas_terceros.html', required_specialty: 'importador_ventas' }
                    ]
                }
            ]
        }
    ...
    window.renderErpSidebar = (profileData = null) => {
            ...
            if (module.type === 'accordion') {
                const newGroups = module.groups.map(group => {
                    const newItems = group.items.filter(item => {
                        if (item.required_specialty && !especialidadesEmpresa.includes(item.required_specialty)) return false;
                        return true;
                    });
        ...
        // 2. Filtrado por Rutas Asignadas (Solo si NO es ADMIN)
